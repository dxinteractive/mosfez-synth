...
`;async function offlineRender($){const yn=$,{initialParams:_e,dspNode:sn,events:ln}=yn,mn=ao(yn,["initialParams","dspNode","events"]),_n=sn.serialize();return offlineRender$1(yt({functionString:`
    ...
    `,props:{initialParams:_e,dspNodeSerialized:_n,events:ln}},mn))}let buffer;async function experimentalOfflineRender($){console.log("events",$);const _e=$.find(isInputStopEvent);if(!_e){console.log("no stop event found");return}const sn=48e3,ln=await offlineRender({channels:1,sampleRate:sn,length:_e.time*sn,initialParams,dspNode:synthDspNode,events:$});buffer=await toAudioBuffer$1(ln,liveAudioContext)}function playLastRender(){if(!buffer){console.log("no buffer to play");return}playBuffer(buffer,liveAudioContext)}client.createRoot(document.getElementById("root")).render(jsx(React.StrictMode,{children:jsx(Demo,{})}));function Demo(){return jsxs("div",{className:classes$7.demo,children:[jsx(Surface,{onSurfaceNoteEvent:handleSurfaceEvent}),jsx(Sidebar,{handleDestroy,handleRecord:()=>synth.inputEvents.record(),handleRecordStop:()=>{const $=synth.inputEvents.stop();experimentalOfflineRender($)},handlePlay:()=>playLastRender()})]})}
